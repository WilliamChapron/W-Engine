...
glm::vec3 randomVectorInRange(float min, float max) {
    // Générer des valeurs aléatoires pour chaque composant du vecteur
...
int main()
{
    ...
    cube2Collider->m_aabb.Init(cube2Geometry->vertices);

    cube2Transform->SetPosition(glm::vec3(4.5f, 0.0f, 0.0f));
    cube2Transform->SetScale(glm::vec3(0.5f, 2.0f, 1.0f));


    // Transforms and colliders
    Transform* cubeTransform = new Transform();
    cubeTransform->SetScale(glm::vec3(3.0f, 1.0f, 1.0f));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Update first cube
        cubeTransform->SetRotation(glm::vec3(rotate, rotate, rotate));
        glm::mat4 cubeWorld = cubeTransform->GetWorldMatrix();
        cubeCollider->m_obb.UpdateGlobalBounds(cubeWorld);

        // Update second cube
        //cube2Transform->SetRotation(glm::vec3(0.0f, -(rotate*3), 0.0f));
        glm::mat4 cube2World = cube2Transform->GetWorldMatrix();
        glm::vec3 randomVec = randomVectorInRange(3.0f, 8.0f);
        cube2Transform->SetPosition(glm::vec3(translate, 0.0f, 0.0f));
        cube2Transform->SetRotation(glm::vec3(rotate, rotate, rotate));
        cube2Collider->m_aabb.UpdateGlobalBounds(cube2World);
        ...
        // TEST COLLISION
        physicSystem->TestAABBvsOBB_CollisionOnly(cubeCollider->m_obb, cube2Collider->m_aabb);
        std::vector<Eigen::Vector3d> collisionPointsOBB1, collisionPointsOBB2;



        glm::mat4 identityMatrix = glm::mat4(1.0f);
        // STORE CORNER FOR 2 OBJECT

        // Object 1
        std::vector<Eigen::Vector3d>& corners = cubeCollider->m_obb.GetCorners();
        ...
        glm::vec3* corners2 = cube2Collider->m_aabb.transformedCorners;
        std::vector<float> lineVertices2;
        for (int i = 0; i < 8; ++i) {
            lineVertices2.push_back(corners2[i].x);  
            lineVertices2.push_back(corners2[i].y);  
            lineVertices2.push_back(corners2[i].z);  
        }
        ...
        // 
        wfsh->UpdateMatrices(identityMatrix, view, projection);
        renderer.DrawBoundingBoxCorners(lineVertices2, indices2, wfsh);
