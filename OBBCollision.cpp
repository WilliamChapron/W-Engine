...
void ProcessKeyboardMovement(GLFWwindow* window, Transform* cubeTransform, float deltaTime) {
    float moveSpeed = 2.0f;
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        cubeTransform->SetPosition(cubeTransform->GetPosition() + glm::vec3(0.0f, moveSpeed * deltaTime, 0.0f));
    }
...
int main()
{
    ...
    cube2Material->SetShader(pshader);
    ...
    cube2Transform->SetPosition(glm::vec3(3.5f, 0.0f, 0.0f));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ProcessKeyboardMovement(window, cubeTransform, deltaTime);
        ...
        cube2Collider->m_obb.UpdateGlobalBounds(cube2World);
        ...
        //bool collision = physicSystem->TestOBBvsOBB_CollisionWithCorners(
        //    cube2Collider->m_obb, cubeCollider->m_obb, 
        //    collisionPointsOBB1, collisionPointsOBB2
        //);




        // Store corners positions and correct indices for draw line bounding box for BOX COLLISION
        ...
        std::vector<Eigen::Vector3d> cube2Corners = cube2Collider->m_obb.GetCorners();
        ...
        wfsh->UpdateMatrices(identityMatrix, view, projection);
        renderer.DrawBoundingBoxCorners(rectangleVertices, indices, wfsh);
